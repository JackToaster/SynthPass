// AES-128 block cipher, ported to plain C from rweather/Crypto's
// AESCommon/AES128 classes (arduinolibs "Crypto" library, MIT licensed,
// see LICENSE). PROGMEM/AVR-specific bits dropped since they're no-ops
// off AVR.

#include "aes128.h"
#include <stddef.h>

// AES S-box (http://en.wikipedia.org/wiki/Rijndael_S-box)
static const uint8_t sbox[256] = {
    
};

// AES inverse S-box (http://en.wikipedia.org/wiki/Rijndael_S-box)
static const uint8_t sbox_inverse[256] = {
    
};

// Rcon(i), 2^i in the Rijndael finite field, for i = 0..10.
static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36
};

// Constants to correct Galois multiplication for the high bits
// that are shifted out when multiplying by powers of two.
static const uint8_t K[8] = {
    0x00,
    0x1B,
    (0x1B << 1),
    (0x1B << 1) ^ 0x1B,
    (0x1B << 2),
    (0x1B << 2) ^ 0x1B,
    (0x1B << 2) ^ (0x1B << 1),
    (0x1B << 2) ^ (0x1B << 1) ^ 0x1B
};

#define gmul2(x)    (t = ((uint16_t)(x)) << 1, \
                     ((uint8_t)t) ^ (uint8_t)(0x1B * ((uint8_t)(t >> 8))))
#define gmul4(x)    (t = ((uint16_t)(x)) << 2, ((uint8_t)t) ^ K[t >> 8])
#define gmul8(x)    (t = ((uint16_t)(x)) << 3, ((uint8_t)t) ^ K[t >> 8])

#define OUT(col, row)   output[(col) * 4 + (row)]
#define IN(col, row)    input[(col) * 4 + (row)]

static void sub_bytes_and_shift_rows(uint8_t *output, const uint8_t *input) {
    OUT(0, 0) = sbox[IN(0, 0)];
    OUT(0, 1) = sbox[IN(1, 1)];
    OUT(0, 2) = sbox[IN(2, 2)];
    OUT(0, 3) = sbox[IN(3, 3)];
    OUT(1, 0) = sbox[IN(1, 0)];
    OUT(1, 1) = sbox[IN(2, 1)];
    OUT(1, 2) = sbox[IN(3, 2)];
    OUT(1, 3) = sbox[IN(0, 3)];
    OUT(2, 0) = sbox[IN(2, 0)];
    OUT(2, 1) = sbox[IN(3, 1)];
    OUT(2, 2) = sbox[IN(0, 2)];
    OUT(2, 3) = sbox[IN(1, 3)];
    OUT(3, 0) = sbox[IN(3, 0)];
    OUT(3, 1) = sbox[IN(0, 1)];
    OUT(3, 2) = sbox[IN(1, 2)];
    OUT(3, 3) = sbox[IN(2, 3)];
}

static void inverse_shift_rows_and_sub_bytes(uint8_t *output, const uint8_t *input) {
    OUT(0, 0) = sbox_inverse[IN(0, 0)];
    OUT(0, 1) = sbox_inverse[IN(3, 1)];
    OUT(0, 2) = sbox_inverse[IN(2, 2)];
    OUT(0, 3) = sbox_inverse[IN(1, 3)];
    OUT(1, 0) = sbox_inverse[IN(1, 0)];
    OUT(1, 1) = sbox_inverse[IN(0, 1)];
    OUT(1, 2) = sbox_inverse[IN(3, 2)];
    OUT(1, 3) = sbox_inverse[IN(2, 3)];
    OUT(2, 0) = sbox_inverse[IN(2, 0)];
    OUT(2, 1) = sbox_inverse[IN(1, 1)];
    OUT(2, 2) = sbox_inverse[IN(0, 2)];
    OUT(2, 3) = sbox_inverse[IN(3, 3)];
    OUT(3, 0) = sbox_inverse[IN(3, 0)];
    OUT(3, 1) = sbox_inverse[IN(2, 1)];
    OUT(3, 2) = sbox_inverse[IN(1, 2)];
    OUT(3, 3) = sbox_inverse[IN(0, 3)];
}

static void mix_column(uint8_t *output, const uint8_t *input) {
    uint16_t t;
    uint8_t a = input[0];
    uint8_t b = input[1];
    uint8_t c = input[2];
    uint8_t d = input[3];
    uint8_t a2 = gmul2(a);
    uint8_t b2 = gmul2(b);
    uint8_t c2 = gmul2(c);
    uint8_t d2 = gmul2(d);
    output[0] = a2 ^ b2 ^ b ^ c ^ d;
    output[1] = a ^ b2 ^ c2 ^ c ^ d;
    output[2] = a ^ b ^ c2 ^ d2 ^ d;
    output[3] = a2 ^ a ^ b ^ c ^ d2;
}

static void inverse_mix_column(uint8_t *output, const uint8_t *input) {
    uint16_t t;
    uint8_t a = input[0];
    uint8_t b = input[1];
    uint8_t c = input[2];
    uint8_t d = input[3];
    uint8_t a2 = gmul2(a);
    uint8_t b2 = gmul2(b);
    uint8_t c2 = gmul2(c);
    uint8_t d2 = gmul2(d);
    uint8_t a4 = gmul4(a);
    uint8_t b4 = gmul4(b);
    uint8_t c4 = gmul4(c);
    uint8_t d4 = gmul4(d);
    uint8_t a8 = gmul8(a);
    uint8_t b8 = gmul8(b);
    uint8_t c8 = gmul8(c);
    uint8_t d8 = gmul8(d);
    output[0] = a8 ^ a4 ^ a2 ^ b8 ^ b2 ^ b ^ c8 ^ c4 ^ c ^ d8 ^ d;
    output[1] = a8 ^ a ^ b8 ^ b4 ^ b2 ^ c8 ^ c2 ^ c ^ d8 ^ d4 ^ d;
    output[2] = a8 ^ a4 ^ a ^ b8 ^ b ^ c8 ^ c4 ^ c2 ^ d8 ^ d2 ^ d;
    output[3] = a8 ^ a2 ^ a ^ b8 ^ b4 ^ b ^ c8 ^ c ^ d8 ^ d4 ^ d2;
}

static void key_schedule_core(uint8_t *output, const uint8_t *input, uint8_t iteration) {
    output[0] = sbox[input[1]] ^ rcon[iteration];
    output[1] = sbox[input[2]];
    output[2] = sbox[input[3]];
    output[3] = sbox[input[0]];
}

void aes128_set_key(aes128_ctx *ctx, const uint8_t key[AES128_KEY_SIZE]) {
    uint8_t *schedule = ctx->schedule;

    for (uint8_t i = 0; i < 16; i++) {
        schedule[i] = key[i];
    }

    // Expand the key schedule until we have 176 bytes of expanded key.
    uint8_t iteration = 1;
    uint16_t n = 16;
    uint8_t w = 4;
    while (n < 176) {
        if (w == 4) {
            key_schedule_core(schedule + 16, schedule + 12, iteration);
            schedule[16] ^= schedule[0];
            schedule[17] ^= schedule[1];
            schedule[18] ^= schedule[2];
            schedule[19] ^= schedule[3];
            iteration++;
            w = 0;
        } else {
            schedule[16] = schedule[12] ^ schedule[0];
            schedule[17] = schedule[13] ^ schedule[1];
            schedule[18] = schedule[14] ^ schedule[2];
            schedule[19] = schedule[15] ^ schedule[3];
        }

        schedule += 4;
        n += 4;
        w++;
    }
}

void aes128_encrypt_block(const aes128_ctx *ctx, uint8_t output[AES128_BLOCK_SIZE], const uint8_t input[AES128_BLOCK_SIZE]) {
    const uint8_t *roundKey = ctx->schedule;
    uint8_t state1[16];
    uint8_t state2[16];

    for (uint8_t i = 0; i < 16; i++) {
        state1[i] = input[i] ^ roundKey[i];
    }
    roundKey += 16;

    for (uint8_t round = AES128_ROUNDS; round > 1; round--) {
        sub_bytes_and_shift_rows(state2, state1);
        mix_column(state1,      state2);
        mix_column(state1 + 4,  state2 + 4);
        mix_column(state1 + 8,  state2 + 8);
        mix_column(state1 + 12, state2 + 12);
        for (uint8_t i = 0; i < 16; i++) {
            state1[i] ^= roundKey[i];
        }
        roundKey += 16;
    }

    sub_bytes_and_shift_rows(state2, state1);
    for (uint8_t i = 0; i < 16; i++) {
        output[i] = state2[i] ^ roundKey[i];
    }
}

void aes128_decrypt_block(const aes128_ctx *ctx, uint8_t output[AES128_BLOCK_SIZE], const uint8_t input[AES128_BLOCK_SIZE]) {
    const uint8_t *roundKey = ctx->schedule + AES128_ROUNDS * 16;
    uint8_t state1[16];
    uint8_t state2[16];

    for (uint8_t i = 0; i < 16; i++) {
        state1[i] = input[i] ^ roundKey[i];
    }
    inverse_shift_rows_and_sub_bytes(state2, state1);

    for (uint8_t round = AES128_ROUNDS; round > 1; round--) {
        roundKey -= 16;
        for (uint8_t i = 0; i < 16; i++) {
            state2[i] ^= roundKey[i];
        }
        inverse_mix_column(state1,      state2);
        inverse_mix_column(state1 + 4,  state2 + 4);
        inverse_mix_column(state1 + 8,  state2 + 8);
        inverse_mix_column(state1 + 12, state2 + 12);
        inverse_shift_rows_and_sub_bytes(state2, state1);
    }

    roundKey -= 16;
    for (uint8_t i = 0; i < 16; i++) {
        output[i] = state2[i] ^ roundKey[i];
    }
}

void aes128_clear(aes128_ctx *ctx) {
    volatile uint8_t *d = (volatile uint8_t *)ctx->schedule;
    for (size_t i = 0; i < sizeof(ctx->schedule); i++) {
        d[i] = 0;
    }
}
